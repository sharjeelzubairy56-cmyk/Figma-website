...
function Cart() {
    ...
    const getBasePrice = (item) => {
        const itemIndex = typeof item === 'object' ? item.index : item;
        return products[itemIndex]?.price || 120;
    };
    ...
    const subtotal = cartItems.reduce((sum, item) => sum + getBasePrice(item) * value, 0);
            ...
            <div className="mx-10 h-auto border-[#F0F0F0] border rounded-[1%]">
                {cartItems.length === 0 ? (
                    ...
                    cartItems.map((item, idx) => {
                        const itemIndex = typeof item === 'object' ? item.index : item;
                        const itemSize = typeof item === 'object' ? item.size : 'Small';

                        return (
                            <div className="p-6" key={idx}>
                                <div className="flex mb-5 gap-4">
                                    <div className=" h-70 w-100">
                                        <img src={products[itemIndex]?.image || "shirt.jpg"} height="100%" width="100%" alt="product" />
                                    </div>
                                    <div className=" w-full">
                                        <div className=" h-14 w-auto flex gap-4">
                                            <div className=" h-14 w-[70%] flex items-center justify-start font-black text-4xl pl-10">
                                                {products[itemIndex]?.title || "T-Shirt with tape details"}
                                            </div>
                                            <div className=" h-14 w-[30%] flex items-center justify-end pr-28 cursor-pointer" onClick={() => handleDelete(idx)}>
                                                <img src="delete.png" height="50%" width="17%" alt="delete" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex">
                                            <div className=" h-14 w-[70%] pl-10 flex items-center justify-start text-2xl tracking-tight font-black">
                                                ${getBasePrice(itemIndex) * value}
                                            </div>
                                            <div>
                                                <button className="cursor-pointer mt-4 ml-6 w-[250%] h-[20%] bg-[#F0F0F0] border-0 text-gray-600 gap-3">
                                                    <button onClick={() => { setValue(value > 1 ? value - 1 : 1); setPrice(base * (value - 1)); }} className="cursor-pointer h-auto text-10x1 bg-[#F0F0F0] border-0 text-black font-black">➖</button>
                                                    {value}
                                                    <button onClick={() => { setValue(value + 1); setPrice(base * (value + 1)); }} className="cursor-pointer h-[10%] text-10x1 bg-[#F0F0F0] border-0 text-black font-black">➕</button>
                                                </button>
                                            </div>
                                        </div>
